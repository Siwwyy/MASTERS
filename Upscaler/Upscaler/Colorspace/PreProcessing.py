...
def preprocessing_pipeline(
    hdr_tens: TensorType, exposure: Union[TensorType, float] = Exposure_Coefficient
) -> TensorType:
    # 1./Exposure -> tonemap -> inverse gamma correction
    hdr_tens *= 1.0 / exposure
    hdr_tens[:] = tonemap_reinhard(hdr_tens)
    return gamma_correction(
        hdr_tens, 1.0 / Gamma_Coefficient
    )  # inverse gamma correction


# Depreprocessing of inputs, Gamma -> Detonemap -> Exposure multiplication
def depreprocessing_pipeline(
    ldr_tens: TensorType, exposure: Union[TensorType, float] = Exposure_Coefficient
) -> TensorType:
